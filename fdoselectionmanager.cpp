...
bool FdoSelectionManager::nativeEventFilter(const QByteArray& eventType, void* message, long int* result)
{
    ...
    if (responseType == XCB_CLIENT_MESSAGE) {
        ...
        if (ce->type == Xcb::atoms->opcodeAtom) {
            qCDebug(SNIPROXY) << "Received system tray message: opcode=" << ce->data.data32[1] 
                              << "window=" << ce->data.data32[2];
        ...
        const auto sniProx = m_proxies.value(damagedWId, nullptr);

        // Don't assert - the window might have been undocked already
        if(sniProx) {
            sniProx->update();
            xcb_damage_subtract(QX11Info::connection(), m_damageWatches[damagedWId], XCB_NONE, XCB_NONE);
        } else {
            qCDebug(SNIPROXY) << "Received damage event for already undocked window" << damagedWId;
        }
    }
...
void FdoSelectionManager::dock(xcb_window_t winId)
{
    ...
    // Validate window ID
    if (winId == 0 || winId == XCB_WINDOW_NONE) {
        qCWarning(SNIPROXY) << "Invalid window ID" << winId << "- ignoring dock request";
        return;
    }
    ...
    QScopedPointer<xcb_get_geometry_reply_t> geom(xcb_get_geometry_reply(c, cookie, &error));
    
    if (error || geom.isNull()) {
        qCWarning(SNIPROXY) << "Window" << winId << "does not exist - ignoring dock request";
        free(error);
        return;
    }
...
void FdoSelectionManager::undock(xcb_window_t winId)
{
    ...
    // Clean up damage watch
    if (m_damageWatches.contains(winId)) {
        xcb_damage_destroy(QX11Info::connection(), m_damageWatches[winId]);
        m_damageWatches.remove(winId);
    }
