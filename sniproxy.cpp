    ...
    m_windowId(wid)
{
    ...
    xcb_create_window (c,                          /* connection    */
                    XCB_COPY_FROM_PARENT,          /* depth         */
                     ...
                     -s_embedSize * 2, -s_embedSize * 2,  /* x, y - position offscreen */
    ...
    // Set WM_CLASS so compositors can identify and create window rules for this
    const char wm_class[] = "xembedsniproxy\0xembedsniproxy";
    ...
    // Set window name
    const char wm_name[] = "xembed-sni-proxy";
    ...
    */
    ...
    NETWinInfo wm(c, m_containerWid, screen->root, NET::WMWindowType, NET::Properties2());
    wm.setWindowType(NET::Utility);  // Set window type to utility
    ...
    // Additional EWMH hints for Wayland compositors
    wm.setDesktop(NETWinInfo::OnAllDesktops);
#endif
...
void SNIProxy::sendClick(uint8_t mouseButton, int x, int y)
{
    ...
    // Move back offscreen
    const uint32_t offscreenVals[2] = { static_cast<uint32_t>(-s_embedSize * 2), static_cast<uint32_t>(-s_embedSize * 2) };
    xcb_configure_window(c, m_containerWid, XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y, offscreenVals);
